...
### ⭐ 시그널 출처가 두 가지다 — 이게 표본 크기를 결정한다

| 러너 | 시그널 출처 | 표본 상한 | 2주 운영 시 |
|---|---|---|---|
| `run_exit_backtest` · `run_100d` | `signals.csv`를 **읽는다** | 기록된 시그널 이력 | **24건** |
| **`run_model_backtest`** | 모델을 돌려 **생성한다** | **가격 데이터 길이** | **1000일 × 종목 수** |

`--window 1000`을 줘도 결과가 2주치였던 이유가 이것이다. 시그널 파일이
2주치면 구간을 늘려도 그 안에 든 시그널이 전부다. **1000일을 보려면
`run_model_backtest`를 써야 한다.**

```bash
python -m stock_auto.backtest.run_model_backtest --days 1000 --price-dir ~/price_cache
```

이 러너는 과거 일봉에 스코어링 모델을 다시 돌려 "그때 시그널이 났을
것인가"를 재구성한다. 24종목 × 1000일이면 시그널이 **800~900건** 나온다.

> ⚠️ 생성된 시그널은 **재현된 모델**의 산출이다. 스코어링 원본이 이
> 저장소에 없어 문서 명세로 다시 구현했으므로 프로덕션 시그널과 완전히
> 같지 않다. 가정 14개는 `docs/MODEL_REPRODUCTION.md`에 있고, 재현도는
> `--validate-against signals.csv`로 숫자로 확인할 수 있다.
>
> **절대 수치를 프로덕션 성과로 읽지 말고**, "보유상한 1일이 수익을
> 깎는가" 같은 **구조적 상대 비교**에 쓴다.
